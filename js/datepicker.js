...
function getMonthYearString(dt) {
	return [ 'January', 'February', 'March', 'April', 'May', 'June', 'July',
			'August', 'September', 'October', 'November', 'December' ][dt
			.getMonth()]
			+ ' ' + dt.getFullYear();
}
...
function chooseDate(e) {
	...
	var div = targ.parentNode.parentNode.parentNode.parentNode.parentNode; // Find
	// the
	// div
	var idOfTextbox = div.getAttribute('datepickertextbox'); // Get the
	// textbox id
	// which was
	// saved in the
	// div
	var textbox = document.getElementById(idOfTextbox); // Find the textbox now
	if (targ.value == '<' || targ.value == '>' || targ.value == '<<'
			|| targ.value == '>>') { // Do they want the change the month?
...
function parseMyDate(d) {
	...
	var a = d.split('-');
	if (a.length != 3)
		return new Date(d); // Missing 2 dashes
	...
	if (a[1] == 'Dec')
		m = 11;
	if (m < 0)
		return new Date(d); // Couldn't find the month
...
function createCalendar(div, month) {
	...
	lastYearBn.setAttribute('date', new Date(month.getFullYear(), month
			.getMonth() - 12, 1, 0, 0, 0, 0).toString());
	var td = topRow.insertCell(-1);
	var lastMonthBn = document.createElement('input');
	lastMonthBn.type = 'button'; // Have to immediately set the type due to
	// IE
	td.appendChild(lastMonthBn);
	lastMonthBn.value = '<';
	lastMonthBn.onclick = chooseDate;
	lastMonthBn.setAttribute('date', new Date(month.getFullYear(), month
	...
	nextMonthBn.setAttribute('date', new Date(month.getFullYear(), month
			.getMonth() + 1, 1, 0, 0, 0, 0).toString());
	var td = topRow.insertCell(-1);
	...
	nextYearBn.setAttribute('date', new Date(month.getFullYear(), month
			.getMonth() + 12, 1, 0, 0, 0, 0).toString());
	...
	var today = new Date();
	date = new Date(month.getFullYear(), month.getMonth(), 1, 0, 0, 0, 0); // Starting
	// at
	// the
	// 1st
	// of
	// the
	// month
	var extras = (date.getDay() + 6) % 7; // How many days of the last month
...
//			inp.setAttribute('date',getDateString(date));
			inp.setAttribute('date', getISODateString(date));
			...
			if (!isNaN(selected) && date.getDate() == selected.getDate()
	...
	// At the end, we do a quick insert of the newly made table, hopefully to
	// remove any chance of screen flicker
...
function showDatePicker(idOfTextbox) {
	...
	div.setAttribute('datepickertextbox', idOfTextbox); // Remember the textbox
	// id in the div
...
function insertAfter(newItem, existingItem) {
	...
	} else { // In case the existingItem has no sibling after itself, append
		// it
...
/*
 * onDOMReady Copyright (c) 2009 Ryan Morr (ryanmorr.com) Licensed under the MIT
 * license.
 */
	...
	var fireDOMReady = function() {
		if (!ready) {
			...
			if (document.removeEventListener)
				document.removeEventListener("DOMContentLoaded", onStateChange,
						false);
